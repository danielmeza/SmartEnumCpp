 ...
 */
 ...
 public:
     explicit SmartEnumSwitchBuilder(const EnumType& value) : value_(value), handled_(false), lastMatch_(false) {}
 
     /**
      * @brief Checks if the enum matches the candidate.
      */
     SmartEnumSwitchBuilder& When(const EnumType& candidate) {
      ...
      */
     SmartEnumSwitchBuilder& Then(const std::function<void()>& action) {
      ...
      */
     void Default(const std::function<void()>& action) {
         if (!handled_) {
             action();
         }
     }
  ...
  */
 template<typename EnumType>
 SmartEnumSwitchBuilder<EnumType> SwitchOn(const EnumType& enumValue) {
